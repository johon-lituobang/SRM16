...
simulatedbatchWeibull_asymptoticbias<-foreach(batchnumber = (1:length(allkurtWeibull)), .combine = 'rbind') %dopar% {
  ...
  SWAHlmean1<-SWAHLmean(x=sortedx,orderlist1_sorted2=orderlist1_AB2_asymptotic,orderlist1_sorted3=orderlist1_AB3_asymptotic,orderlist1_sorted4=orderlist1_AB4_asymptotic,orderlist1_sorted5=orderlist1_AB5_asymptotic,interval=8,batch="auto")
  THL1<-THL(x=sortedx,orderlist1_sorted2=orderlist1_AB2_asymptotic,interval=4,batch="auto",boot=TRUE)
  rqm1<-rqm(x=sortedx,sorted = TRUE)
  momentsx<-unbiasedmoments(x=sortedx)
  sortedx<-c()
  
  allrawmoBias<-c(
    firstbias=abs(c(Huberx,SMWM9,midhinge1,SWA8,SWAHlmean1,THL1,rqm1)-momentsx[1])/(sqrt(momentsx[2]))
  )
  
  all1<-t(c(kurtx,Huberx,SMWM9,midhinge1,SWA8,SWAHlmean1,THL1,rqm1,targetall,momentsx,allrawmoBias))
}
